...
(function (global, factory) {
  ...
  $(window).on(Event$1.LOAD_DATA_API, () => {
  ...
  class Dropdown {
    ...
    static _dataApiKeydownHandler(event) {
      ...
      const items = [].slice.call(parent.querySelectorAll(Selector$4.VISIBLE_ITEMS)).filter(item => $(item).is(':visible'));
    ...
    _triggerBackdropTransition() {
      if (this._config.backdrop === 'static') {
        ...
        $(this._element).one(Util.TRANSITION_END, () => {
          this._element.classList.remove(ClassName$5.STATIC);
  ...
  class Toast {
    ...
    _setListeners() {
      $(this._element).on(Event$a.CLICK_DISMISS, Selector$a.DATA_DISMISS, () => this.hide());
    }
